...
const SearchResult = () => {
  const postSearchedState = useSelector((state) => state.postsSearch);
  const dispatch = useDispatch();
  const router = useRouter();
  const { keyword } = router.query;

  useEffect(() => {
    dispatch(ACTIONS.postSearch.getPostSearched(keyword));
  }, []);
  const result = postSearchedState.listSearched;
  const Loading = postSearchedState.loading;

  return (
    <PostLayout search={false} back={true}>
      {Loading != true ? (
