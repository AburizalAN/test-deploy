...
export default function PlantBased() {
  ...
  useEffect(() => {
    ...
    dispatch(ACTIONS.recipe.getListSubcategory(category));
    dispatch(ACTIONS.recipeCatalog.getListRecipeCatalog(category));
  }, [router.isReady]);

  return (
    <Box>
      <TopNavDetail title={category} disableSearch />
      <CategoryLink categories={categoryList} headCategory={category} />
