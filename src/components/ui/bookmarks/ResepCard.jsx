...
const ResepCard = ({ item, mutate }) => {
  const d = new Date(item.created_at);
  const monthName = MONTH[d.getMonth()];
  const day = d.getDate();

  const { data } = useSWR(
    `${MICROSERVICE_BASE_URL}/recipe?slug=${item.url_key}`,
    fetcher,
    {
  ...
  const removeBookmark = async () => {
    const res = await fetch(`/api/bookmark/remove-recipe`, {
      method: 'POST',
      body: JSON.stringify({
  ...
  return (
    <Link href={`/resep/${data[0].slug}`} passHref>
      <Wrapper>
        <img
